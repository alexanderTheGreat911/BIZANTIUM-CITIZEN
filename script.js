// Data kartu hewan (duplikat untuk pair)
const animals = ['🐶', '🐱', '🐭', '🐹', '🐰', '🦊', '🐻', '🐼', '🐨', '🐯', '🦁', '🐮']; // Tambah jika perlu untuk level sulit
...
function endGame() {
    gameStarted = false;
    clearInterval(timerInterval);
    // Hitung skor final: dasar + 100 per match - penalti moves dan timer
    let finalScore = Math.max(0, 1000 + (matchedPairs * 100) - (moves * 50) - (timer * 10));
...
function updateDisplay() {
    ...
    // Update skor real-time: dasar + 100 per match - penalti (tanpa timer di real-time, tapi timer update di interval)
    let tempScore = Math.max(0, 1000 + (matchedPairs * 100) - (moves * 50) - (timer * 10));
